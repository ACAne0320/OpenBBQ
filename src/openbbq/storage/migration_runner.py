...
USER_TABLES = frozenset({"providers", "credentials"})
...
def run_schema_migrations(path: Path, kind: DatabaseKind) -> None:
    ...
    required_tables = PROJECT_TABLES if kind == "project" else USER_TABLES
    if "alembic_version" not in existing_tables and required_tables <= existing_tables:
        command.stamp(config, "head", tag=kind)
        return
