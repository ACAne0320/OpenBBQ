...
const api = {
  ...
  getLlmProviderSecret: (name: string) => ipcRenderer.invoke("openbbq:get-llm-provider-secret", name),
  getLlmProviderModels: (name: string) => ipcRenderer.invoke("openbbq:get-llm-provider-models", name),
  testLlmProviderConnection: (input: unknown) => ipcRenderer.invoke("openbbq:test-llm-provider-connection", input),
